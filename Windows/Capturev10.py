...
def count_cameras():
    """This inits cams and then counts the connected cams. Required as windows opencv doesnt have a list cams function"""
    nc = 0
    ...
    for i in range(17):
        try:
            ...
            cam.read()
            nc += 1
            camList.append('camera %s' % i)  # populate a list for drop down boxes
        except:
            print '%s camera(s) detected.' % nc
            return nc-1
...
class MyFrame(wx.Frame):
    ...
    def __init__(self):
        super(MyFrame, self).__init__(None, id=wx.ID_ANY, title="LeafPy Capture", size=(1000, 600),
                                      name="MyFrame")
    ...
    def on_click(self, event):
        ...
        selection = camList.index(selection)
    ...
    def camerasnp(self, sel, saveas):
        cam = cv2.VideoCapture(sel)
    ...
    def timelapser(self, time_sec, loops, matrix, save_dlg):
        ...
        while counter < loops:
            ...
            for cam_shoot in matrix:
                if cam_shoot == "":
                    continue
                else:
                    for snap in range(0, 3):  # takes 3 images at each time point
                        self.camerasnp(camList.index(cam_shoot), '%s/Plate%s-%s-c%s.png' % (save_dlg, (matrix.index(cam_shoot)+1), counter,snap))
    ...
    def on_click17(self, event):
            ...
            all_selected_cameras = [self.combo_box1.GetStringSelection(), self.combo_box2.GetStringSelection(),
                                    self.combo_box3.GetStringSelection(), self.combo_box4.GetStringSelection(),
                                    self.combo_box5.GetStringSelection(), self.combo_box6.GetStringSelection(),
                                    self.combo_box7.GetStringSelection(), self.combo_box8.GetStringSelection(),
                                    self.combo_box9.GetStringSelection(), self.combo_box10.GetStringSelection(),
                                    self.combo_box11.GetStringSelection(), self.combo_box12.GetStringSelection(),
                                    self.combo_box13.GetStringSelection(), self.combo_box14.GetStringSelection(),
                                    self.combo_box15.GetStringSelection(), self.combo_box16.GetStringSelection()]
            ...
            for val in all_selected_cameras:
                if val == 'Select camera':
                    all_selected_cameras[all_selected_cameras.index(val)] = ''

            if all(val == '' for val in all_selected_cameras):
            ...
            if run_time < 60:  # this prevents errors where time taken for images is longer than interval
                ...
                complete_dialog = wx.MessageDialog(None, message="Image capture complete", style=wx.OK)
